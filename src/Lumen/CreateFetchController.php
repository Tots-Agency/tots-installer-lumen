...
class CreateController extends CreateBaseController
{
    /**
     * Path del archivo a tener de base
     * @var string
     */
    protected $filePath = __DIR__ . '/../../data/lumen/create_fetchbyid_controller.txt';
    /**
    ...
    public function run()
    {
        ...
        file_put_contents($this->savePath . '/' . $this->getCamelCase($this->name) . '/FetchByIdController.php', $this->file);

        // Agregamos route
        $this->addRoute('get', $this->name . 's/{id}', $this->getCamelCase($this->name) . '\\FetchByIdController');
    }
}